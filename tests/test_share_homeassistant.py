...
RUN_SCRIPT = (
    REPO_ROOT / "tailscale/rootfs/etc/s6-overlay/s6-rc.d/share-homeassistant/run"
)
FIXTURES = REPO_ROOT / "tests/fixtures"


@pytest.fixture(scope="session")
def bashio_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    destination = tmp_path_factory.mktemp("dependencies") / "bashio"
    subprocess.run(
        [
            "git",
            "clone",
...
def run_share(
    ...
    *,
    ...
    capture_serve_config: bool = False,
...
def share_commands(log_path: Path) -> list[tuple[str, ...]]:
...
def test_accepts_http_origin_with_warning(tmp_path: Path, bashio_dir: Path) -> None:
    config_path = write_config(tmp_path, sites=["http://example.com"])
    result, data_root, _ = run_share(tmp_path, config_path, bashio_dir)
...
def test_accepts_origin_with_path(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_accepts_port_out_of_range(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_accepts_valid_port_and_writes_assetlinks(
    tmp_path: Path, bashio_dir: Path
) -> None:
...
def test_deduplicates_and_sorts_sites(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_share_mode_matches_dal(tmp_path: Path, bashio_dir: Path) -> None:
    config_path = write_config(
        tmp_path, share_mode="serve", sites=["https://example.com"]
    )
    result, _, _ = run_share(tmp_path, config_path, bashio_dir)
    assert result.returncode == 0
...
def test_share_mode_matches_dal_funnel(tmp_path: Path, bashio_dir: Path) -> None:
    config_path = write_config(
        tmp_path, share_mode="funnel", sites=["https://example.com"]
    )
    status_json = json.dumps(
        {
            "Self": {
    ...
    result, _, _ = run_share(tmp_path, config_path, bashio_dir, status_json=status_json)
    assert result.returncode == 0
    # Now we use a single serve set-raw command (even for funnel mode)
    assert share_commands(tmp_path / "tailscale.log") == [("serve", "set-raw")]


def test_funnel_requires_capability(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_creates_data_directory(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_empty_sites_no_assetlinks(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_empty_sites_no_dal_serve_handler(tmp_path: Path, bashio_dir: Path) -> None:
    """When no sites are configured, serve config should not include DAL path handler."""
    config_path = write_config(tmp_path, sites=[])
    result, _, serve_config_path = run_share(
        tmp_path, config_path, bashio_dir, capture_serve_config=True
    )
    assert result.returncode == 0
    ...
    serve_config = json.loads(serve_config_path.read_text(encoding="utf-8"))
    handlers = serve_config["Web"]["test-device.tailnet.ts.net:443"]["Handlers"]
    assert "/.well-known/" not in handlers


def test_dal_serve_config_includes_handler(tmp_path: Path, bashio_dir: Path) -> None:
...
def test_dal_serve_config_has_correct_structure(
    tmp_path: Path, bashio_dir: Path
) -> None:
    """Verify the full ServeConfig structure when DAL is configured."""
    config_path = write_config(
        tmp_path, share_mode="serve", sites=["https://example.com"], share_port=443
    )
    result, _, serve_config_path = run_share(
        tmp_path, config_path, bashio_dir, capture_serve_config=True
    )
